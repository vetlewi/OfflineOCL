...
{
public:
    FIFO_events(size_t max = 4096) : maxSize( max ) { }

    // Push an event. If the FIFO is full, wait for some time, then try again.
    ...
    // Pop an event from the FIFO. If FIFO is empty, wait before checking agin.
    Event Pop(){
        Event r;
        while ( true ){
